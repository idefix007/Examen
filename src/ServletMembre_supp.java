...
@WebServlet(name = "ServletMembre_supp", urlPatterns = {"/supp-membre"})
public class ServletMembre_supp extends HttpServlet {
...
*/
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       /* try {
            BaseDeDonnees.supprimeMembre(request.getParameter("id"));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        getServletContext().getRequestDispatcher("/vues/listeMembres.jsp").forward(request, response);*/
        //String id = request.getParameter("membres");

        try {

            BaseDeDonnees.supprimeMembre(request.getParameter("membres"));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        response.sendRedirect("/membre");
